...
$(function() {
	...
	var newLine=function(){
		...
		var tr=$("<tr></tr>")
					...
					.append($("<td></td>")
							...
							.append($("<div />").hide().append($("<label />").text("显示顺序")).append($("<input />").css({
								'width':'20px',
								'border':'none',
					...
					.append($("<td></td>").append($("<input />").attr("type","button").val("点击此处设置").css({
						"color":"#A5A5A3",
						"font-size":"12px"
					}).click(floatDiv)))
	...
	var delImg=function (){
		...
		for(var n=0;n<trs.length;n++){
			if($(trs[n]).children(":eq(0)").attr("id")==data){
				showDialog("alert","该标题在流程中已设置关联，不能删除！", "提示信息", 300);
				return false;
			};
		}
			...
			if(activity.subjects && activity.subjects.childList){
				for(var n in activity.subjects.childList){
					...
					if(data==subject.subjectKey){
						showDialog("alert","该标题在["+activity.name+"]中已设置关联，不能删除！", "提示信息", 300);
						return false;
					}
				}
			}
	...
	var checkTitle=function (){
		...
		if(dataArray[data]==null){
			...
			showDialog("alert","该显示名称与其他名称发生冲突,请重设", "提示信息", 300);
			$(this).val("");
			$(this).focus();
			return false;
		}
	...
	if(fromParentParam.isShow){
		...
		for(var n in defs){
			...
			$(tr).append($("<td></td>").append($("<input />").attr("type","button").val("点击此处设置").css({
				"color":"#A5A5A3",
				"font-size":"12px"
			}).click(floatDiv)));
	...
	$.ajax({
				...
				error : function() {
					showDialog("alert","请求信息出错", "提示信息", 300);
	...
	$("#confirm").bind("click",function(){
		...
		if(fromParentParam.isShow){
			...
			if($(".rightTableArea tbody tr").length>1 && $(".joinLabelArea input").val()==""){
				showDialog("alert","请设置连接符", "提示信息", 300);
				return false;
			}
			...
			if(fromParentParam.refSubjectJoin!=null&&fromParentParam.refSubjectField!=""){
				...
				for(var n in field){
					...
					.append($("<tr></tr>")
							...
							.append($("<td></td>").append($("<img/>").attr("src",WFlow.fullWebPath+"/jsp/bdo/procdef/html/skin/images/del.jpg")))
							.attr("id",fields[0]));
				}
	...
	function initField1(treeNode){
		if(treeNode.area){
			...
			for(var n in treeNode.area){
				...
				$(tr).append($("<td></td>").append($("<img/>").attr("src",WFlow.fullWebPath+"/jsp/bdo/procdef/html/skin/images/add.jpg")));
			...
			$(".leftTableArea img").click(function(){
				if($("#con2 select").val()==null){
					showDialog("alert","请选择标题", "提示信息", 300);
				...
				for(var i =0;i<trs.length;i++){
					if($(trs[i]).children("td:eq(0)").attr("id")==$("#con2 select").val()){
						showDialog("alert","标题["+$("#con2 select").children("option:selected").text()+"]已添加关联!", "提示信息", 300);
						return false;
					}
				}
				var trr=$(tr).clone();
				...
				$(image).attr("src",WFlow.fullWebPath+"/jsp/bdo/procdef/html/skin/images/del.jpg")
				$(image).unbind("click");
			...
			$(".leftTableArea img").click(function(){
				...
				for(var i =0;i<trs.length;i++){
					if($(trs[i]).attr("id")==$(tr).attr("id")){
						showDialog("alert","不能重复添加", "提示信息", 300);
	...
	parent.SD_Dialog.showFloatDiv=function(param){
		var url=parent.CFlow.webPath+"/jsp/bdo/procdef/html/form/formarea.jsp";
		parent.SD_Dialog.showWindow1("选择表单域", url, 500, 390,param, afterCloseForm);
		function afterCloseForm(backData){
			if(backData&&backData.name){
				$(setButton).val(backData.name);
				$(setButton).attr("id",backData.id);	
			}else{
				$(setButton).val("点击此处设置");
				$(setButton).attr("id","");
			}
