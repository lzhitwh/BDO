...
$(function($){
    var newDiv = function(nodeId,targetNode){
        ...
        var div2=$("<div />").addClass("ruleArea").append($("<ul />").append($("<li />")
                .append($("<input />").attr("type","button").val("选择表单域").click(function(){
                ...
                .append($("<select></select>")
                        .append("<option selected='selected' value='contains'>包含</option>")
                        .append("<option value='uncontains'>不包含</option>")
                        .append("<option value='matches'>匹配</option>")
                ...
                .append($("<input />").attr("type","text").val("输入值").click(function(){
                    if($(this).val()=="输入值"){
                        $(this).val("");
                    }
                }).keydown(function(e){
                ...
                .append($("<div />").addClass("addImg").click(function(){
                    ...
                    if(scopeType=="contains" || scopeType=="uncontains" || scopeType=="matches"){
                        ...
                        if(!valuePatrn.exec(value)){
                            showDialog("alert","值只能为数字！", "提示信息", 300);
                            return false;
                        }
                        rule += "Double.valueOf("+itemId+")"+scopeType+parseInt(value);
                        }
                        }else{
                            showDialog("alert","请输入值！", "提示信息", 300);
...
//                    return false;
    ...
    parent.SD_Dialog.showAreaDiv2=function(param){
        ...
        parent.SD_Dialog.showWindow1("选择表单域", url, 500, 390,param, afterCloseForm1);
    ...
    function validate(exp, targetName){
        ...
        for(var i=0;i<exps.length;i++){
            ...
            if(!strPatrn.exec(tmpExp) && !numPatrn.exec(tmpExp)){
                showDialog("alert","["+gname+"-->"+targetName+"]的表达式格式不正确！", "提示信息", 300);
                return false;
            }
        }
        return true;
