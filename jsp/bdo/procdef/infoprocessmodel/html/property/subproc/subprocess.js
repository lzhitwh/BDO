...
(function($){
	$(function(){
		...
		$("#callSplitRule input:button").bind("click",function(){
			var url=WFlow.webPath+"/jsp/bdo/procdef/infoprocessmodel/html/property/condition/branch.jsp";
			if(actTargetNodeList==null){
				showDialog("alert","没有找到目标环节！", "提示信息", 300);
				return false;
			}else{
				var f=0;
				...
				if(f==1){
					showDialog("alert","仅存在一个目标环节，无需设置分支条件！", "提示信息", 300);
					return false;
				}
			}
			var param={frontAct:wfdCallData.model, targetNodeList:actTargetNodeList};
			showWindow("设置分支条件", url, 600, 425,param, afterCloseBranchForm);
		});
		...
		$("#callSubBtn").click(function (){
			var url=WFlow.webPath+"/jsp/bdo/procdef/infoprocessmodel/html/property/subproc/showprocess.jsp";
			var param={"procDefId":WFModel.process.id};
			showWindow("选择子流程", url, 550, 455,param, afterClose);
	...
	$("#dataMapBtn").click(function(){
		if($("#callSub input:text").attr("id")){}else{
			showDialog("alert","请先选择子流程", "提示信息", 300);
			$("#wfdCallTab1 a").trigger("click");
			return false;
		}
		var url=WFlow.webPath+"/jsp/bdo/procdef/infoprocessmodel/html/property/subproc/datamap.jsp";
		...
		var fnc=function(){
			if(subproc.length==0){
				showDialog("alert","子流程未设置相关数据", "提示信息", 300);
				return false;
			}
			var param={
			...
			showWindow("数据映射", url, 450, 425,param, afterCloseDataMap);
		};
...
function setBratchDivInSub(activity){
	...
	for(var actId in nodeDic){
		...
		if(node.type!="start"&& node.type!="end" && node.id!=activity.id){
	...
	if(flag){
		$("#callSplitRule input:text").val("已设置");
	}else{
...
function getAndParserModelContent(subproc,procDefId,fnc) {
	...
	$.ajax({
		...
		error:function(){
			showDialog("alert","请求流程模型时出错！", "提示信息", 300);
		}
	});
};
