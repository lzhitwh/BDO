...
(function($) {
	...
	WFGraph.isStartSeqExist = function(nodeDefId) {
		for(var key in WFGraph.flowDiagramDic) {
			...
			if (nodeDefId == fromNodeModel.model.id && fromNodeModel.model.type == "start") {
				return true;
			}
		}
		return false;
	...
	WFGraph.isEndSeqExist = function(nodeDefId) {
		for(var key in WFGraph.flowDiagramDic) {
			...
			if (nodeDefId == toNodeModel.model.id && toNodeModel.model.type == "end") {
				return true;
			}
		}
		return false;
	...
	WFGraph.getDirection = function (node, point) {
		if ( !WFGraph.useStraightLine && !point.direction ) {
	...
	function mouseClick( currentSeq ) {
		...
		for (var c in graphList) {
			$([graphList[c].node]).bind("click", function(e) {
				...
				currentSeq.sourceModelId = currentSeq.model.sourceNode.model.id;
