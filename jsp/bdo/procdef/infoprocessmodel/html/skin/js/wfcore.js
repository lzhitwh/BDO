...
(function($){
	$(function() {
		// 解析url中的参数，放入全局变量WFlow.parameter中。
		parseUrl();
	 ...
	 * WF_SHOW_PROPERTY：选中节点、迁移线或单击绘图区的空白区域时，通知业务展现业务属性面板<br>
	 */
	WFlow.event = {
	...
	WFlow.getTargetNodeInfo = function(actId) {
		...
		for(var key in WFGraph.flowDiagramDic) {
			...
			if (actId == fromNodeModel.model.id) {
				if(line.targetNode.model.type!="end"){
					var targetNode={};
	...
	WFlow.setRelease = function() {
		...
		$.ajax({
			...
			error:function(){
				showDialog("alert","请求已发布的环节数据时出错！", "提示信息", 300);
			}
		});
	}
	...
	WFlow.validateProcess = function() {
		...
		if (!hasStart) {
			retObj.push("必须绘制开始环节!");
		}
		if (!hasEnd) {
			retObj.push("必须绘制结束环节!");
		}
		
		for(var s in startActDic) {
			var sModel = startActDic[s];
			if (!sModel.hasLine) {
				retObj.push("开始环节【" + sModel.model.name + "】必须有迁出线!");
			}
		}
		for(var e in endActDic) {
			var eModel = endActDic[e];
			if (!eModel.hasLine) {
				retObj.push("结束环节【" + eModel.model.name + "】必须有迁入线!");
			}
		}
		for(var o in otherActDic) {
			var oModel = otherActDic[o];
			if (!oModel.hasLine) {
				retObj.push("【" + oModel.model.name + "】必须有迁入或迁出线!");
			}
		}
							...
							for(var p in nodeModel.potentialOwnerList) {
								if (p) {
									...
									if (pTmp.language && (pTmp.language.indexOf("creator") != -1 || 
										...
										retObj.push("【" + tModel.name + "】的参与者不能选择为创建人、发送人、历史环节处理人!");
										isErr = true;
										break;
									}
								}
	...
	WFlow.validateNodeName = function(name) {
		if (!name) {
			showDialog("alert","环节名称不能为空！", "提示信息", 300);
			return false;
		}
		
		if (isNodeNameExist(name)) {
			// 提示用户不能修改
			showDialog("alert","环节["+name+"]已经存在！", "提示信息", 300);
			return false;
		}
		return true;
	}
	...
	WFlow.validateNodeId = function(id) {
		if (!id) {
			showDialog("alert","环节ID不能为空！", "提示信息", 300);
			return false;
		}
		if (isNodeIdExist(id) == true) {
			// 提示用户不能修改
			showDialog("alert","环节ID["+id+"]已经存在！", "提示信息", 300);
			return false;
		}
		return true;
	}
