(function($) {
	...
	WFGraph.drawCallActivity = function(nodeInfo) {
		...
		var nodeTaskRect = WFGraph.canvas.rect(0, 0, nodeInfo.width, nodeInfo.height, nodeInfo.r).attr(nodeInfo.attr).hide();
		var inner=WFGraph.canvas.path(getPath(nodeInfo)).attr({"stroke":"#333333"}).hide();
