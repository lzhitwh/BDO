...
(function($){
	WFGraph.drawLine = function ( lineInfo ) {
		...
		var graphList = {};
		var seqBackGraph = WFGraph.canvas.path( "M0,0L10,10" ).attr(
				{ stroke : "#fff", 'stroke-width' : 10 }).hide();
		graphList[ lineInfo.id+"_backSeq" ] = seqBackGraph;
		// 迁移线
		var seqLine = WFGraph.canvas.path( "M0,0L10,10" ).attr( lineInfo.attr ).hide();
		graphList[ lineInfo.id ] = seqLine;
		
		var seqArrow = WFGraph.canvas.path( "M0,0L10,10" ).attr( lineInfo.attr ).hide();
		graphList[ lineInfo.id+"_arrowSeq" ] = seqArrow;
		
		var seqArrow2 = WFGraph.canvas.path( "M0,0L10,10" ).attr( lineInfo.attr ).hide();
