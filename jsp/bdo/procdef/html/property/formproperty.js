$(function(){
    ...
    if(CFProcess.isSetLocal){
        $("#procLocalOpt input:text").val("已设置");
    }
    ...
    $("#procFormName input:button").bind("click",function(){
        $.ajax({
            ...
            success:function (data){
                if(data.errMessage){
                    showDialog("alert",data.errMessage, "提示信息", 400);
                }else{
                    if(data.pageCount>0){
                        var url=CFlow.webPath+"/jsp/bdo/procdef/html/form/showForm.jsp";
                        var param={formType:"PCForm"};
                        showWindow("选择表单", url, 500, 455,param, afterCloseForm);
                    }else{
                        showDialog("alert","未查询到表单，请先定义表单", "提示信息", 400);
                    }
            ...
            for(var n in activities){
                ...
                if(!activity.isSetForm){
                    ...
                    if(activity.formId!=backData.formId){
                        act.push(activity);
//                    showConfirm("是否将变化同步到"+activity.name+"?", changeForm, "提示信息", 300, false);
        ...
        function dealAct(){
            if(index==actleng)
                return false;
            showConfirm("是否将变化同步到"+act[index].name+"?", confirmclick, "提示信息", 300, false,cancelclick);
        }
    ...
    $("#procLocalOpt input:button").bind("click",function(){
        if(!$("#procFormName input:text").attr("id")){
            showDialog("alert","请先设置表单！", "提示信息", 300);
            $("#procTab1 a").trigger("click");
            return false;
        }
        var url=CFlow.webPath+"/jsp/bdo/procdef/html/form/localopt.jsp";
        var param={"show":false,"CFProcess":CFProcess};
        showWindow("选择操作", url, 550, 500,param, afterCloseLocalOpt);
    });
    ...
    function afterCloseLocalOpt(ret){
        if(ret){
            ...
            var url=CFlow.webPath+"/jsp/bdo/procdef/html/form/actsyn.jsp";
            ...
            if(flag)
                showWindow("选择同步环节", url,  650, 500,param, afterSyn);
        }
    }
    //流程默认标题设置
    $("#procDefaultSubject input:button").click(function(){
        ...
        var url=CFlow.webPath+"/jsp/bdo/procdef/html/form/formarea.jsp";
        var param={
            isdefault:true,
        ...
        showWindow1("选择流程默认标题", url, 500, 505,param, acProcDefaultSubject);
    ...
    $("#procTypeSubject input:button").click(function(){
        ...
        $.ajax({
            ...
            success : function(data) {
                var oldSubjects={};
                if(data.length==0){
                    showDialog("alert","请先设置流程类型标题", "提示信息", 300);
                    return false;
                }else{
                    var url=CFlow.webPath+"/jsp/bdo/procdef/html/subject/proctypesubject.jsp";
                    ...
                    showWindow("选择流程类型标题", url, 500, 375,param, acProcTypeSubject);
                }
    ...
    $("#procDefSubject input:button").click(function(){
        ...
        var url=CFlow.webPath+"/jsp/bdo/procdef/html/subject/subjectdef.jsp";
        var subjects=new Array();
        var oldSubjects={};
        if(CFProcess.subjects){
            var subs=CFProcess.subjects.childList;
        ...
        showWindow("选择流程定义标题", url, 600, 450,param, acProcDefSubject);
    ...
    function dealWithSynAct(addList, oldSubjects, type, subjectData ){
        ...
        if(flag){
            var url=CFlow.webPath+"/jsp/bdo/procdef/html/form/actsubjectsyn.jsp";
            ...
            showWindow("选择同步环节", url, 570, 475,param, afterSubjectSyn);
        }
    }
});