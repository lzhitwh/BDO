...
$(function(){
	$("#processName input").bind("change",function(){
		...
		var nameV = true;
		if (!name) {
			showDialog("alert","流程名称不能为空！", "提示信息", 300);
			nameV = false;
		}
		if (nameV) {
			var id="";
			...
			var idV = true;
			if (!id || id == "") {
				showDialog("alert","流程ID不能为空！", "提示信息", 300);
				idV = false;
			}
			if (idV) {
				// 如果修改了流程ID、流程名称
	...
	$("#processLimit select").bind("change",function(){
		...
		if(unit=="N"){
			$("#processWarn span").text("自然日");
		}else{
			$("#processWarn span").text("工作日");
		}
		updateProcessData();
	});
	...
	$("#dataObject input:button").click(function(){
			var url=WFlow.webPath+"/jsp/bdo/procdef/infoprocessmodel/html/property/process/dataobj.jsp";
			var param={"dataobj":WFlow.getDataObject()};
			showWindow("设置相关数据", url, 330, 360,param, aftDataobj);
