	...
	WFGraph.addEventToNode = function( currentNode ) {
		...
		for ( var m in graphList ) {
			$( [graphList[m].node] ).hover(function(e) {
				...
				WFEvent.trigger( "bizClickNode", data ); // --> wfevent.js
...
//				$("#dispatcher select").focus();
		...
		function a() {
			...
			if( currentNode.widget.type == NODE_TYPE.ACTION_NODE )
				return true;
			
			if( currentNode.widget.type == NODE_TYPE.CONTROL_NODE )
				switch( currentNode.widget.id ) {
					case "start":
					case "end":
						if( "1" == WFlow.parameter["editEvent"] ) {
							result = true;
						}
						break;
				}
		...
		if (a()) {
			...
			currentNode.showName = function(N) {
				...
				$(taskTextDiv).offset({top: N.y+dy+3, left: N.x+dx+5});
	...
	WFGraph.showNodeAddition = function(currentNode) {
		// 显示四个锚点
		WFGraph.showNodeAction(currentNode); // --> wf.act.action.js
		
		// “开始”节点不能删除
		if( currentNode.widget.id == "start")
			return;
	...
	WFGraph.drawLineWhenClick = function( currentNode, mousePoint ) {
		...
		var selectedLine = WFGraph.drawingLine;
		if ( selectedLine ) {
			var selectedModel = selectedLine.model;
			if ( selectedModel.type == "sequenceFlow" ) {
				if ( currActType == "start" ) {
					showDialog( "alert","不能指向开始环节！", "提示信息", 300 );
					selectedLine.remove();
					WF.removeDrawingLine();
					return false;
				}
				if ( currActType == "end" && WFGraph.isEndSeqExist( currActId ) ) {
					showDialog( "alert","结束环节的迁入线已经存在！", "提示信息", 300 );
					selectedLine.remove();
					WF.removeDrawingLine();
					return false;
				}
				var fromNode = selectedModel.sourceNode.model;
				if ( fromNode.type == "start" && currActType == "end" ) {
					showDialog( "alert","不能在开始环节和结束环节之间画线！", "提示信息", 300 );
					selectedLine.remove();
					WF.removeDrawingLine();
					return false;
				}
				if ( fromNode.id == currActId ) {
					showDialog( "alert","不能在同一环节画线！", "提示信息", 300 );
					selectedLine.remove();
					WF.removeDrawingLine();
					return false;
				}
				if ( WFGraph.isNodeSeqExist( fromNode.id, currActId ) ) {
					// 两点之间已经存在顺序流、不再绘制
					showDialog( "alert","["+fromNode.name+"]和["+currModel.name+"]之间的迁移线已经存在！", "提示信息", 300 );
					selectedLine.remove();
					WF.removeDrawingLine();
					return false;
				}
		...
		if ( currActType == "start" && WFGraph.isStartSeqExist( currActId ) ) {
...
//					}
	...
	function showHRefLine(y) {
		if (hRefLine) {
			...
			hRefLine = WFGraph.canvas.path("M0 "+y+"H"+dx).toFront().show();
		}
	}
	...
	function showVRefLine(x) {
		...
		if (vRefLine) {
			...
			vRefLine = WFGraph.canvas.path("M"+x+" 0V"+dy).toFront().show();
		}
	}
