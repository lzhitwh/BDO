...
//	var refLine = null;
	...
	function drawPoint(seqLine, data) {
		...
		var anchorDot = WFGraph.canvas.rect(point.x, point.y, h, h).attr(attr).show();
		if (anchorDot) {
		...
		this.resetLinePoint = function(x, y) {
			...
			switch (type) {
		        case "from":
                    ...
                    var fromFlag = true;
                    if (selectedNode) {
                    	if (selectedNode.model.type == "end") {
                    		fromFlag = false;
                    	}
...
//		        		showRefLine(fDotPos, tDotPos);
		        ...
		        case "to":
                    ...
                    var toFlag = true;
                    if (selectedNode) {
                    	if (selectedNode.model.type == "start") {
                    		toFlag = false;
                    	}
...
//		} else {
//			refLine = WFGraph.canvas.path("M"+vX+",0 "+"L0,"+vY).show();
//		}
//	}
