...
$(function(){
    ...
    $("#branchCondiId input:button").bind("click",function(){
        var url=CFlow.webPath+"/jsp/bdo/procdef/html/condition/branch.jsp";
        
        var frontNode = WFlow.getFrontAct(CFlow.getSelectedId());
        if(frontNode==null){
            showDialog("alert","请为分支环节设置上一环节！", "提示信息", 300);
            return false;
        }else{
            frontAct=CFProcess.activities.childList[frontNode.id];
            if(frontAct.formId){
            }else{
                showDialog("alert","请先为上一环节设置表单！", "提示信息", 300);
                return false;    
            }
        }
        ...
        showWindow("设置分支条件", url, 600, 425,param, afterCloseForm);
    });
...
CFlow.initGataWay =function(name){
    ...
    if(flag){
        $("#branchCondiId input:text").val("已设置");
    }else{
