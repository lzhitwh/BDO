...
$(function(){
	...
	$("#activityLimit select").bind("change",function(){
		...
		if(unit=="N"){
			$("#activityWarn span").text("自然日");
		}else{
			$("#activityWarn span").text("工作日");
		}
		updateActivityData();
	});
	...
	$("#participants input:button").bind("click",function(){
		...
		if(partList){
			...
			showWindow("选择参与者", WFlow.webPath+"/jsp/bdo/procdef/infoprocessmodel/html/property/task/participantForm.jsp", 640, 515,param, afterCloseForm);
		}
	});
			...
			success : function(data) {
				if(data && data.success){
					if(data.partList.length==0){
						showDialog("alert","未配置参与者页面，请配置！", "提示信息", 300);						
					}else{
